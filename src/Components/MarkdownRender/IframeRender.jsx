...
const LOCALSTORAGE_HIGHLIGHTS_KEY = "web-marker-highlights-iframe";
...
class IframeRender extends React.Component {
  ...
  componentDidMount() {
    this.iframeRef.current.onload = () => {
      ...
      contentWindow.document.addEventListener("mouseup", this.mouseupListener);
      contentWindow.addEventListener("pointermove", this.mouseMoveListener);
  ...
  componentWillUnmount() {
    const contentWindow = this.iframeRef.current.contentWindow;
    contentWindow.document.removeEventListener("mouseup", this.mouseupListener);
    contentWindow.removeEventListener("pointermove", this.mouseMoveListener);
  ...
  render() {
    ...
    return (
      <div className="markdown-render-root">
        ...
        <iframe ref={this.iframeRef} src={`${process.env.PUBLIC_URL}/iframe.html`} style={{ width: '100%', height: '100vh' }} />
      </div>
    );
  }
